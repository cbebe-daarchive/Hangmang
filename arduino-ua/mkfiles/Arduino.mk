...
#	0.9   2012-10-15  H James Hoover
#			  - factored out compiler flags 
#			  - also create .s files and a .map
#			  - moved ard-parse-boards into mkfiles
#
########################################################################
#
...
#
#   make upload-0	- load to arduino on port 0
#   make upload-1	- load to arduino on port 1
#   You need to run arduino-port-select to map the serial ports attached
#   to connected arduinos to ports 0 and 1 in this makefile
#
########################################################################
#
...
ifndef PARSE_BOARD
PARSE_BOARD = $(ARDUINO_UA_DIR)/mkfiles/ard-parse-boards --boards_txt=$(BOARDS_TXT)
endif

ifndef ARDUINO_PORT_SELECT
ARDUINO_PORT_SELECT = $(ARDUINO_UA_DIR)/bin/arduino-port-select
endif
...
#######
# generate asm .s files for investigation
# used as a implication style guard for || pipe, so false means do it, 
# and true means skip and return result of command 
# This is a nice example of why p => q is defined as !p or q
# 
ifndef GENERATE_ASM 
  ASM_SWITCH=true
else
...
CPPFLAGS      = -mmcu=$(MCU) -DF_CPU=$(F_CPU) -DARDUINO=$(ARDUINO_VERSION) \
			...
			$(SYS_INCLUDES) -w -Wall $(CPP_OPTIMIZE)\
			-ffunction-sections -fdata-sections
CFLAGS        = -std=gnu99 $(C_OPTIMIZE)
CXXFLAGS      = -fno-exceptions -Wall -Werror
ASFLAGS       = -mmcu=$(MCU) -I. -x assembler-with-cpp
LDFLAGS       = -mmcu=$(MCU) -Wl,--gc-sections $(LD_OPTIMIZE)

# Expand and pick the first port if that's all we have to work
# with.  But it is better to have the ports and associated lock
# files defined so that we can't accidently upload when we are
# running a serial monitor.  Also, we want to be able to upload
# to multiple arduinos.
# ARD_PORT      = $(firstword $(wildcard $(ARDUINO_PORT)))
#
# so use arduino-select-port to configure the available ports

# need to generalize to many ports, but 2 is sufficient for most cases
ifndef ARD_PORT_0
...
$(OBJDIR)/libs/%.o: $(ARDUINO_LIB_PATH)/%.c
	...
	$(ASM_SWITCH) || $(CC) -S $(CPPFLAGS) $(CFLAGS) $< -o $(dir $@)$(notdir $<).s

$(OBJDIR)/libs/%.o: $(ARDUINO_LIB_PATH)/%.cpp
	mkdir -p $(dir $@)
	$(CC) -c $(CPPFLAGS) $(CXXFLAGS) $< -o $@
	$(ASM_SWITCH) || $(CC) -S $(CPPFLAGS) $(CXXFLAGS) $< -o $(dir $@)$(notdir $<).s
...
$(OBJDIR)/%.o: %.cc
	...
	$(ASM_SWITCH) || $(CXX) -S $(CPPFLAGS) $(CXXFLAGS) $< -o $(dir $@)$(notdir $<).s
...
# core files
# how do we get the source or target without the prefix?
...
$(OBJDIR)/%.hex: $(OBJDIR)/%.elf
	...
	# Generate the memory map of the program
	$(NM) --numeric-sort $< > $<.map
...
AVRDUDE_ARD_OPTS = -c $(AVRDUDE_ARD_PROGRAMMER) -b $(AVRDUDE_ARD_BAUDRATE) 
...
# disable this reset option and force specifying the port for upload
#upload:		reset raw_upload

# upload defaults to port 0
upload:         upload-0

# upload to one or the other port, make sure the port config files
# are defined.
$(HOME)/.arduino_port_0:
		$(ARDUINO_PORT_SELECT)
...
upload-0:	$(HOME)/.arduino_port_0 upload_0_set upload_check raw_upload
upload-1:	$(HOME)/.arduino_port_1 upload_1_set upload_check raw_upload
...
# we need to figure out how to use the $(error ) makefile thing
# $(error "ERROR: serial-mon has $(ARD_PORT) locked")

# before upload, check that the hex file does not have the infamous !!!
# sequence that causes the old bootloader to hang.
...
raw_upload:	$(TARGET_HEX)
		...
		$(AVRDUDE) $(AVRDUDE_COM_OPTS) $(AVRDUDE_ARD_OPTS) \
			 -P $(ARD_PORT) -U flash:w:$(TARGET_HEX):i
